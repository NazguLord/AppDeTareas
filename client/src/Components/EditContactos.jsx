...
const EditContactos = () => {
 ...
 // const [open, onClose ] = useState(false);
  ...
  const contacName = location.pathname.setName;
 
  console.log(contactoId);
  console.log(contacName);
  console.log("ContactoInfo:", state.image);
  ...
  return (
    ...
    <Typography id="modal-modal-description" sx={{ mt: 2 }}>
      ...
      <TextField name='name' onChange={(e) => {setName(e.target.value) }} variant="est?ndard" label="Nombre"  defaultValue={state.name}/>
      ...
      onChange={(e) => {  setNumber(e.target.value) }} variant="est?ndard" defaultValue={state.number}/>           
      <TextField id="est?ndard-multiline-flexible"  label="Email"    name='email' onChange={((e) => setEmail(e.target.value))} variant="est?ndard" defaultValue={state.email}/>
      ...
      { file === null ? (<Chip label={ state.image}   />  ) : (<Chip label={  file.name }   />  )}
    {console.log(file)}     
    {console.log(state.image)}      
    <DialogActions>
      <Button variant="contained" onClick={(e) => {handleClick(e);  }}>Actualizar</Button>
      <Button variant="outlined" color="error" autoFocus component={Link} to={`/contactos`}>Regresar</Button>                   
    </DialogActions>
    </Box>
    </div>
  );
